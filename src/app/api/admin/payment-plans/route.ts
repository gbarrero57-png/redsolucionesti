...
export async function PATCH(req: NextRequest) {
  ...
  if (installment_id) {
    const { data: inst, error: instErr } = await supabaseAdmin
      ...
      .single();
    if (instErr || !inst) return NextResponse.json({ error: 'Installment not found' }, { status: 404, headers: NO_CACHE });

    // Update installments_paid count + next_due_date on parent plan
    const { data: plan } = await supabaseAdmin
      .from('payment_plans')
      .select('installments_paid, installments_total, frequency, payment_installments(due_date, status)')
      .eq('id', (inst as Record<string, unknown>).payment_plan_id)
      .single();

    if (plan) {
      const p = plan as Record<string, unknown>;
      const paid = Number(p.installments_paid) + 1;
      const total = Number(p.installments_total);
      const installs = (p.payment_installments as Array<{ due_date: string; status: string }>) || [];
      const nextPending = installs.filter(i => i.status === 'pending').sort((a, b) => a.due_date.localeCompare(b.due_date))[0];
      await supabaseAdmin
        .from('payment_plans')
        .update({
