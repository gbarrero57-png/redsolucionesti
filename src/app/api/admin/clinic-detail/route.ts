...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Fetch auth emails for each staff member — safe fallback if auth lookup fails
    ...
    // Recent appointments (last 20) — use correct column names: start_time, phone
    const { data: appointments } = await supabaseAdmin
      .from('appointments')
      .select('id, patient_name, phone, start_time, service, status, notes, created_at')
      .eq('clinic_id', clinic_id)
    ...
    // Normalize appointments to match ClinicDrawer expectations
    const normalizedAppts = (appointments || []).map(a => ({
      id: a.id,
      patient_name: a.patient_name,
      patient_phone: a.phone ?? null,       // phone → patient_phone
      date_time: a.start_time ?? null,      // start_time → date_time
      service: a.service,
      status: a.status,
      notes: a.notes ?? null,
    ...
    return NextResponse.json({
      ...
      appointments: {
        counts: apptCounts,
        recent: normalizedAppts,
      },
    }, { headers: NO_CACHE });

  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[clinic-detail]', msg);
    return NextResponse.json({ error: 'Error cargando detalle de clínica' }, { status: 500, headers: NO_CACHE });
  }
}
