...
import { MessageSquare, Calendar, BarChart2, BookOpen, Bot, LogOut, Users, Building2, AlertTriangle } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Determine role IMMEDIATELY from the current path — middleware ensures
  // only superadmins can be on SA_PATHS, so no flash or async wait needed.
  ...
  // Verify role server-side (background, corrects if cookie is stale)
  useEffect(() => {
    fetch('/api/auth/me')
  ...
  // Enforce correct section based on verified role
  useEffect(() => {
    ...
    <div className="flex h-screen bg-gray-950 text-gray-100">
      ...
      {showWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-900 border border-amber-500/30 rounded-2xl p-7 max-w-sm w-full mx-4 shadow-2xl"
            style={{ boxShadow: '0 0 60px -10px rgba(245,158,11,0.2)' }}>
            {/* Top gradient bar */}
      ...
      <aside className="w-64 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col">
        ...
        <div className="px-6 py-5 border-b border-gray-800">
          <div className="flex items-center gap-3">
            ...
            {isSuperadmin && (
              <span className="ml-auto text-[10px] bg-amber-600/20 text-amber-400 border border-amber-600/30 px-1.5 py-0.5 rounded-full font-medium">
                SA
              </span>
            )}
          </div>
